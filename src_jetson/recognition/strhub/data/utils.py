...
from typing import Optional, List, Tuple  # ✅ updated import
...
class BaseTokenizer(ABC):
    ...
    def __init__(self, charset: str, specials_first: Tuple = (), specials_last: Tuple = ()) -> None:  # ✅ Tuple
    ...
    def _tok2ids(self, tokens: str) -> List[int]:  # ✅ changed from list[int]
        return [self._stoi[s] for s in tokens]

    def _ids2tok(self, token_ids: List[int], join: bool = True) -> str:  # ✅ changed from list[int]
        tokens = [self._itos[i] for i in token_ids]
        return ''.join(tokens) if join else tokens

    @abstractmethod
    def encode(self, labels: List[str], device: Optional[torch.device] = None) -> Tensor:  # ✅ List[str]
    ...
    @abstractmethod
    def _filter(self, probs: Tensor, ids: Tensor) -> Tuple[Tensor, List[int]]:  # ✅ Tuple
        """Internal method which performs the necessary filtering prior to decoding."""
        raise NotImplementedError

    def decode(self, token_dists: Tensor, raw: bool = False) -> Tuple[List[str], List[Tensor]]:  # ✅ Tuple[List[str], List[Tensor]]
